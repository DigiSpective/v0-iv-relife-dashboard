-- IV RELIFE Fixed Database Setup Script
...
-- Owners and backoffice can view all users (simplified to avoid recursion)
...
CREATE POLICY "Owners and backoffice can view all users" ON public.users
  FOR SELECT USING (role IN ('owner', 'backoffice'));

-- Owners and backoffice can manage users (simplified to avoid recursion)
...
CREATE POLICY "Owners and backoffice can manage users" ON public.users
  FOR ALL USING (role IN ('owner', 'backoffice'));
...
CREATE POLICY "Owners and backoffice can manage invites" ON public.invite_tokens
  FOR ALL USING (EXISTS (SELECT 1 FROM public.users u WHERE u.id = auth.uid() AND u.role IN ('owner', 'backoffice')));
...
CREATE POLICY "retailers_retailer_view" ON public.retailers
  FOR SELECT USING (id = (SELECT retailer_id FROM public.users WHERE id = auth.uid()));
...
CREATE POLICY "locations_retailer_manage" ON public.locations
  FOR ALL USING (retailer_id = (SELECT retailer_id FROM public.users WHERE id = auth.uid()));
...
CREATE POLICY "locations_location_view" ON public.locations
  FOR SELECT USING (id = (SELECT location_id FROM public.users WHERE id = auth.uid()));
...
CREATE POLICY "customers_location" ON public.customers
  FOR ALL USING (
    ((auth.jwt() ->> 'role') = 'location_user' AND (auth.jwt() ->> 'location_id')::uuid = primary_location_id)
    OR ((auth.jwt() ->> 'role') = 'owner')
  );
...
CREATE POLICY "customer_contacts_parent_scope" ON public.customer_contacts
  FOR ALL USING (
    EXISTS (SELECT 1 FROM public.customers WHERE customers.id = customer_contacts.customer_id AND (
      ...
      OR ((auth.jwt() ->> 'location_id')::uuid = customers.primary_location_id AND (auth.jwt() ->> 'role') = 'location_user')
    ))
  );
...
CREATE POLICY "customer_documents_scope" ON public.customer_documents
  FOR ALL USING (
    ...
    OR ((auth.jwt() ->> 'location_id')::uuid = location_id AND (auth.jwt() ->> 'role') = 'location_user')
  );
...
CREATE POLICY "orders_location_manage" ON public.orders
  FOR ALL USING (location_id = (SELECT location_id FROM public.users WHERE id = auth.uid()));
...
CREATE POLICY "shipping_quotes_retailer_location" ON public.shipping_quotes
  FOR ALL USING (
    ((auth.jwt() ->> 'role') = 'retailer' AND (auth.jwt() ->> 'retailer_id')::uuid = retailer_id)
    OR ((auth.jwt() ->> 'role') = 'location_user' AND (auth.jwt() ->> 'location_id')::uuid = location_id)
    OR ((auth.jwt() ->> 'role') = 'owner')
...
CREATE POLICY "audit_logs_admin_view" ON public.audit_logs
  FOR SELECT USING (EXISTS (SELECT 1 FROM public.users u WHERE u.id = auth.uid() AND u.role IN ('owner', 'backoffice')));
...
CREATE POLICY "outbox_owner_view" ON public.outbox
  FOR SELECT USING (EXISTS (SELECT 1 FROM public.users u WHERE u.id = auth.uid() AND u.role = 'owner'));
...
CREATE POLICY "file_metadata_retailer_view" ON public.file_metadata
  FOR SELECT USING (retailer_id = (SELECT retailer_id FROM public.users WHERE id = auth.uid()));
...
CREATE POLICY "file_metadata_location_view" ON public.file_metadata
  FOR SELECT USING (location_id = (SELECT location_id FROM public.users WHERE id = auth.uid()));
